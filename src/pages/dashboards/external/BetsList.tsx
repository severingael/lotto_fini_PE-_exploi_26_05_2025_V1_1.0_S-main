          ...
          {filteredBets.map((bet) => (
            <div key={bet.id} className="bg-white rounded-lg shadow-sm p-6">
              ...
              <div className={`mt-4 p-3 rounded-lg flex items-center gap-2 ${
                bet.status === 'pending' ? 'bg-yellow-50' :
                bet.status === 'won' ? 'bg-green-50' : 'bg-red-50'
              }`}>
                {getStatusIcon(bet.status)}
                <p className={`text-sm ${
                  bet.status === 'pending' ? 'text-yellow-800' :
                  bet.status === 'won' ? 'text-green-800' : 'text-red-800'
                }`}>
                  {bet.status === 'pending' && 'En attente du résultat'}
                  {bet.status === 'won' && 'Pari gagné !'}
                  {bet.status === 'lost' && 'Pari perdu'}
                </p>
              </div>
            </div>
          ))}
